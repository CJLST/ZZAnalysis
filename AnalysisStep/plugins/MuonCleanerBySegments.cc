...
namespace modules {
  ...
  class MuonCleanerBySegmentsT : public edm::EDProducer {
    ...
    private:
      ...
      typedef std::pair<const pat::Muon *, const pat::Muon *> MuonPointerPair;
...
modules::MuonCleanerBySegmentsT<T>::isBetterMuon(const T &mu1, const T &mu2) const {
    if (!defaultBestMuon_) {
      MuonPointerPair pair =std::make_pair( &mu1, &mu2 );
        return bestMuonSelector_(pair);
    }
    if (mu2.track().isNull()) return true;
    if (mu1.track().isNull()) return false;
    if ((mu1.userFloat("isPFMuon")>0.5)     != (mu2.userFloat("isPFMuon")>0.5))     return (mu1.userFloat("isPFMuon")>0.5);
