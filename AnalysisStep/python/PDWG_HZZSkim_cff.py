...
DIMUON_KINCUT_HZZSKIM  = ("(max(daughter(0).pt(),daughter(1).pt())>20 && min(daughter(0).pt,daughter(1).pt())>10)")
goodHzzMuons = cms.EDFilter("PATMuonRefSelector", src = cms.InputTag("patMuonsWithTrigger"), cut = cms.string(MUON_CUT_HZZSKIM) )
...
DIELECTRON_KINCUT_HZZSKIM  = ("(max(daughter(0).pt(),daughter(1).pt())>20 && min(daughter(0).pt,daughter(1).pt())>10)")
goodHzzElectrons = cms.EDFilter("PATElectronRefSelector", src = cms.InputTag("patElectronsWithTrigger"), cut = cms.string(ELECTRON_CUT_HZZSKIM) )
hzzKinDiElectrons = cms.EDProducer("CandViewShallowCloneCombiner",
                                   decay       = cms.string("goodHzzElectrons goodHzzElectrons"),
                                   checkCharge = cms.bool(False),
                                   cut         = cms.string(DIELECTRON_KINCUT_HZZSKIM)
                                   )
hzzKinDiElectronsFilter = cms.EDFilter("CandViewCountFilter", src = cms.InputTag("hzzKinDiElectrons"), minNumber = cms.uint32(1) )
hzzMassDiElectrons = hzzKinDiElectrons.clone( cut=cms.string(DIELECTRON_MASSCUT_HZZSKIM) )
hzzMassDiElectronsFilter = hzzKinDiElectronsFilter.clone( src = cms.InputTag("hzzMassDiElectrons") )

########## CROSS SELECTION
DILEPTON_MASSCUT_HZZSKIM = ("mass > 40")
DILEPTON_KINCUT_HZZSKIM  = ("(max(daughter(0).pt(),daughter(1).pt())>20 && min(daughter(0).pt,daughter(1).pt())>10)")
hzzKinCrossLeptons = cms.EDProducer("CandViewShallowCloneCombiner",
                                    decay       = cms.string("goodHzzElectrons goodHzzMuons"),
                                    checkCharge = cms.bool(False),
                                    cut         = cms.string(DILEPTON_KINCUT_HZZSKIM)
                                    )

hzzKinCrossLeptonsFilter = cms.EDFilter("CandViewCountFilter", src = cms.InputTag("hzzKinCrossLeptons"), minNumber = cms.uint32(1) )
hzzMassCrossLeptons = hzzKinCrossLeptons.clone( cut=cms.string(DILEPTON_MASSCUT_HZZSKIM) )
