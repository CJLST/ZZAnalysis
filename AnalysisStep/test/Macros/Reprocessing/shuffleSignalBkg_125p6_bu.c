...
void shuffleSignalBkg_125p6(int flavor, int erg_tev){
	...
	float MC_weight_QQBGGProper[4]={0};
...
//					NGenTotal_weighted[smp][p] += NGenTotal_weighted_perFlavor[smp][p][myf]/NGenTotal_weighted_perFlavor[smp][p][2]*NGenTotal_unweighted_perFlavor[smp][2];
			...
			for(int p=0;p<myNumSamples;p++) sum_NGenTotal_weighted[p] += NGenTotal_weighted[smp][p];
		...
		if(smp>=kqqZZSamples+kggZZSamples+kSignalSamples && smp<kZXSamples+kqqZZSamples+kggZZSamples+kSignalSamples) myTree[smp]->SetBranchAddress("ZXfake_weightProper",&ZXfake_weightProper);
			...
			myTree[smp]->SetBranchAddress("MC_ME_as2mu2e_spin0",MC_ME_as2mu2e_spin0);
...
//				float myxsec = MC_weight_xsec * MC_weight_spin0[p]  / sum_NGenTotal_weighted[p] ;
	...
	while(ctr<nTotal_ggZZ){
		...
		if(ZZMass>=220) nGGZZReWgt += MC_weight*MC_weight_QQBGGProper[0];
		if(ZZMass<140.6 && ZZMass>=105.6) nGGZZReWgt_in160 += MC_weight*MC_weight_QQBGGProper[0];
		if(ZZMass<140.6 && ZZMass>=105.6) nGGZZReWgt160 += MC_weight*MC_weight_QQBGGProper[1];
		if(ZZMass>=220) nQQZZReWgt += MC_weight*MC_weight_QQBGGProper[2];
		if(ZZMass<140.6 && ZZMass>=105.6) nQQZZReWgt_in160 += MC_weight*MC_weight_QQBGGProper[2];
		if(ZZMass<140.6 && ZZMass>=105.6) nQQZZ_DedicatedReWgt += MC_weight*MC_weight_QQBGGProper[3];
	...
	cout << "nGGZZ Raw (mZZ>=220): " << nGGZZ << endl;
	cout << "nGGZZ + qqZZZ (mZZ>=220): " << nGGZZReWgt << endl;
	cout << "nGGZZ Raw (105.6<=mZZ<140.6): " << nGGZZ160 << endl;
