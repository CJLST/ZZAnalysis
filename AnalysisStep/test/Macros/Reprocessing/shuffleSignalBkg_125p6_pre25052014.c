...
//	float MC_CVwrt2mu2e_weight[numSamples];
	...
	float NGenTotal_4GeVcut_weighted[kSignalSamples][numSamples];
	float NGenTotal_4GeVcut_unweighted[kSignalSamples]={0};
	float NGenTotal_4GeVcut_weighted_perFlavor[kSignalSamples][numSamples][nFinalStates];
	float fGen_4GeVcut_BRweight_perFlavor[numSamples][nFinalStates]={{0}};
	float fGen_4GeVcut_BRweight[numSamples]={0};
	float NGenTotal_4GeVcut_unweighted_perFlavor[kSignalSamples][nFinalStates];
	float sum_NGenTotal_4GeVcut_unweighted=0;
	float sum_NGenTotal_4GeVcut_weighted[numSamples]={0};
...
*/
	...
	float nSM_4GeVcut[kSignalSamples]={0};
	float nSM_2mu2e[kSignalSamples]={0};
	float nHypoPredicted[kSignalSamples]={0};
	float nHypo[numSamples+2]={0};
	float nHypoPredicted_4GeVcut[kSignalSamples]={0};
	float nHypo_4GeVcut[numSamples+2]={0};
	float nHypo_AsIf2e2mu[numSamples+2]={0};
	float nGGZZ=0;
	float nQQZZ=0;
	float nQQZZ_Dedicated=0;
	float nGGZZ160=0;
	float nQQZZ160=0;
	float nQQZZ_Dedicated160=0;
	float nZX=0;
	float nGGZZReWgt=0;
	float nGGZZReWgt160=0;
	float nGGZZReWgt_in160=0;
	float nQQZZReWgt=0;
	float nQQZZReWgt_in160=0;
	float nQQZZ_DedicatedReWgt=0;
...
//				MC_weight_spin0[p] *= ( NGenTotal_unweighted_perFlavor[lucky][2]/NGenTotal_weighted_perFlavor[lucky][p][2] );
				...
				float myxsec_4GeVcut = MC_weight_xsec_4GeVcut * MC_weight_4GeVcut_spin0[p]  / sum_NGenTotal_4GeVcut_unweighted ;
//				float myxsec = MC_weight_xsec * MC_weight_spin0[p]  / sum_NGenTotal_weighted[p] ;
				...
				if(MC_CV_weight[p]>=1.5e-5) MC_CV_weight[p] = pow(1.5e-5,2)/MC_CV_weight[p];
				if(MC_CV_4GeVcut_weight[p]>=1.5e-5) MC_CV_4GeVcut_weight[p] = pow(1.5e-5,2)/MC_CV_4GeVcut_weight[p];
