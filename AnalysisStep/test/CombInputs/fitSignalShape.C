...
void all(int selAna =-10,  int channels=0, int categ =-10, int sample = 0 ){
  ...
  for(int i=0; i<maxMassBin;++i){
    fitSignalShapeW(mass[i],id[i],selAna,channels,categ, sample,/* 10.,doSfLepton,*/xLow[i],xHigh[i],bwSigma[i],
    ...
    cout << "scale3 value,error: " << fitValues[7] << " , " << fitErrors[7] << endl;
    scale3[i]=fitValues[7]; scale3[i]=fitErrors[7];
...
//  }
    ...
    string sID = static_cast<ostringstream*>( &(ostringstream() << id[i] ) ) ->str() ;
    string filename = "signal_shape_parametriztion_13TeV_" + ssample + sID + "_" + schannel + "_" + sselAna + "_" + scategory + "." + "yaml" ;
    ofstream outFile;
...
//    {
    ...
    outFile <<"    meanBW : " <<"'"<<fitValues[3]<<"'"<<endl;
    outFile <<"    meanCB : " <<"'"<<fitValues[2]<<"'"<<endl;
    outFile <<"    sigma  : " <<"'"<<fitValues[6]<<"'"<<endl;
    outFile <<"    alpha  : " <<"'"<<fitValues[0]<<"'"<<endl;
    outFile <<"    n      : " <<"'"<<fitValues[4]<<"'"<<endl;
    outFile <<"    alpha2 : " <<"'"<<fitValues[1]<<"'"<<endl;
    outFile <<"    n2     : " <<"'"<<fitValues[5]<<"'"<<endl;
    outFile << endl;
/*    }
    else {
    outFile << scategory<<":"<<endl;
    outFile <<"    <<: *untagged " << endl;     
    }*/
}
  stringstream namefile;
  namefile << "parameters_channel_"<< sselAna<<"_"<<schannel<<"_"<< scategory << ".root";
...
void fitSignalShapeW(int massBin,int id,int selAna, int channels,int categ, int sample, 
 ...
 // ------ root settings ---------
  ...
  if(sample==1) FileName << "root://lxcms03//data3/Higgs/160121/ggH"<<id<<"/ZZ4lAnalysis.root";
  else if(sample==2) FileName << "root://lxcms03//data3/Higgs/160121/VBFH125/ZZ4lAnalysis.root";
  else {
  ...
  for(int k=0; k<nentries; k++){
    ...
    if (selAna == 0) Cat = categoryLegacy(nJets);	    
  ...
  RooRealVar mean("mean","mean of gaussian",0,-15.,15.) ;
...
/*
  RooRealVar mean("mean","mean of DCBall",0. , -10. , 10.) ;
  RooRealVar sigma("sigma","width of gaussian",1.0, -1., 5.); 
  RooRealVar a1("a1","a1",1.5,0.1,5.);
  RooRealVar n1("n1","n1",1.,0.,50.);   
  RooRealVar a2("a2","a2",1.,0.,30.);
  RooRealVar n2("n2","n2",20,1.,50.);   
 */
...
//  //--- Breit-Wigner
  ...
  RooArgSet* params = new RooArgSet(RooArgList(mean3, scale3, mean, sigma, a1, n1, a2, n2));
  ...
  nameFile    << "fitM" << massBin << "_" << sselAna << "_" << ssample << "_" << schannel << "_"<< scategory << ".pdf";
  nameFileC   << "fitM" << massBin << "_" << sselAna << "_" << ssample << "_" << schannel << "_"<< scategory << ".C";
  nameFilePng << "fitM" << massBin << "_" << sselAna << "_" << ssample << "_" << schannel << "_"<< scategory << ".png";
