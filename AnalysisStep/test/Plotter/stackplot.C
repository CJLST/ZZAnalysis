...
//   }
    ...
    lumist = "2.97 fb^{-1}";
  } else if (fabs(lumi-1616)<1.) {
    lumist = "1.62 fb^{-1}";
  } else if (fabs(lumi-5051)<1.) {
    lumist = "5.1 fb^{-1}";
  } else if (fabs(lumi-4968)<1.) {
    lumist = "4.97 fb^{-1}";
  } else if (fabs(lumi-4670)<1.) {
    lumist = "4.67 fb^{-1}";
  } else if (fabs(lumi-4716)<1.) {
    lumist = "4.72 fb^{-1}";
  } else if (fabs(lumi-4348)<1.) {
    lumist = "4.35 fb^{-1}";
  } else if (fabs(lumi-1841)<1.) {
    lumist = "1.84 fb^{-1}";
  } else if (fabs(lumi-4041)<1.) {
    lumist = "4.04 fb^{-1}";
  } else if (fabs(lumi-3570)<1.) {
    lumist = "3.57 fb^{-1}";
  } else if (fabs(lumi-2948)<1.) {
    lumist = "2.95 fb^{-1}";
  } else if (fabs(lumi-2544)<1.) {
    lumist = "2.54 fb^{-1}";
  } else if (fabs(lumi-2200)<1.) {
    lumist = "2.20 fb^{-1}";
  } else if (fabs(lumi-1660)<1.) {
    lumist = "1.66 fb^{-1}";
  } else if (fabs(lumi-1179)<1.) {
    lumist = "1.18 fb^{-1}";
  } else if (fabs(lumi-481)<1.) {
    lumist = "481 pb^{-1}";
  }
  ...
  if (CME == 7 || CME == 8) {
    ...
    text = "#sqrt{s} = 7 TeV, L = 5.1 fb^{-1} ; #sqrt{s} = 8 TeV, L = 19.8 fb^{-1}";
    //  ll->SetTextAlign(32); // align right
...
THStack* replaceDD(THStack* stack, float lumi, TString finalState, TString epoch,  int sel, TString var)
{
  ...
  float  p0 = 0.00439895  ;// = normalisation of landau1  (i.e. 2P2F 2mu2e)
  float  p1 = 195.407     ;//= MPV of Landau1
  float  p2 = 38.9472     ;//= sigma of Landau
  float  p3 = 3.68476     ;//= a  in pol1 = a + bx
  float  p4 = -0.00580439 ;//= b  in pol1 = a + bx
  float  p5 = 1.92769     ;//= normalisation of Landau2  (i.e. 3P1F 2mu2e)
  float  p6 = 110.862     ;//= MPV of Landau2
  float  p7 = 9.59455     ;//= sigma of Laudau2
  float  p8 = 1.          ;//= normalisation of Landau3  (i.e. 2e2mu), set to one by convention
  float  p9 = 129         ;//= MPV of Landau3
  float  p10 = 15         ;//= sigma of Landau3
	...
	Sigma        = 24.3;
      ...
      if (fs==1) { // 4e
        //Legacy
          ...
          p3 = 0 ;//= a  in pol1 = a + bx
          p4 = 0 ;//= b  in pol1 = a + bx
          p5 = 0 ;//= normalisation of Landau2  (i.e. 3P1F 2mu2e)
          p8 = 0 ;//= normalisation of Landau3
          ZjetLandau->SetParameters(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10);
...
*/
  ...
  if (var!="ZZ"&&sel==3) {
    Zjet[0][sel] *= 1; //FIXME hardcoded range!!! ZjetLandau.Integral(121,131)/ZjetLandau.Integral(0,1000)
    Zjet[1][sel] *= 1;
    Zjet[2][sel] *= 1;
  }
...
//    cout << h->GetTitle() << sample << " " << isSignal << " " << noFillSignal << endl;    
	...
	if (sname=="ZZ") h->SetLineColor(kBlue+2);
    if (sname=="Z+jet") h->SetLineColor(kGreen+4);
