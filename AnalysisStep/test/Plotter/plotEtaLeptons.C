...
void plotEtaLeptons(int mass,float ptcut=0,int ich=2,bool fullsorted=0,TCanvas *c1=0x0){
  ...
  for(int j=0;j<4;j++){
    Leppt[j]=0;
    Lepeta[j]=0;
    TString histname;histname.Form("#eta^{%d}_RECO",j+1);
    hReco[j]=new TH1F(histname.Data(),histname.Data(),100,-3,3);
    hReco[j]->SetLineColor(col[j]);
    hReco[j]->SetLineStyle(2);
    hReco[j]->SetFillColor(col[j]);
    hReco[j]->SetFillStyle(3002);
    hReco[j]->GetXaxis()->SetTitle("#eta^{l}_{t} [GeV/c]");
    hReco[j]->GetYaxis()->SetTitle("a.u.");
    hReco[j]->SetLineWidth(2);

    histname.Form("#eta^{%d}_GEN",j+1);
    hGen[j]=new TH1F(histname.Data(),histname.Data(),100,-3,3);
    hGen[j]->SetLineColor(colline[j]);
    hGen[j]->SetLineStyle(2);
    hGen[j]->SetFillColor(0);
    hGen[j]->SetFillStyle(0);
    hGen[j]->GetXaxis()->SetTitle("#eta^{l}_{t} [GeV/c]");
  ...
  for(int i=0;i<4;i++){
    ...
    TString varadd;varadd.Form("GenLep%dEta",i+1);
    tree->SetBranchAddress(varadd.Data(),&Genlepeta[i],&b_genlepeta[i]);
    varadd.Form("Lep%dEta",i+1);
    tree->SetBranchAddress(varadd.Data(),&Lepeta[i],&b_lepeta[i]);
  ...
  for(int i=0;i<nentries;i++){
    ...
    if(genleppt[0]<genleppt[1]){
      ...
      dummy = genlepeta[0];genlepeta[0]=genlepeta[1];genlepeta[1]=dummy;
    }
    if(genleppt[2]<genleppt[3]){
      ...
      dummy = genlepeta[2];genlepeta[2]=genlepeta[3];genlepeta[3]=dummy;    
    }
	...
	for(int j=0;j<3&&sorted;j++){	    
	  if(genleppt[j]<genleppt[j+1]){
	    ...
	    dummy2 = genlepeta[j];genlepeta[j]=genlepeta[j+1];genlepeta[j+1]=dummy2;
	    sorted=false;
	  }
	}
      }
    }
    for(int j=0;j<4;j++)hGen[j]->Fill(genlepeta[j]);
    
    if(iBC<0)continue;
    //if(ZZsel->at(iBC)<70)continue;
    if(ZZsel->at(iBC)<100)continue;
    if(genleppt[3]<ptcut)continue;//cut on pt acceptance!
    if(debug)printf("entry %d/%d\n",i,nentries-1);
    for(int j=0;j<4;j++){
      leppt[j]=Leppt[j]->at(iBC);
      lepeta[j]=Lepeta[j]->at(iBC);
    }
    ...
    if(leppt[0]<leppt[1]){
      ...
      dummy = lepeta[0];lepeta[0]=lepeta[1];lepeta[1]=dummy;
    }
    if(leppt[2]<leppt[3]){
      if(debug)printf("inverting leptons 23\n");
      Float_t dummy = leppt[2];leppt[2]=leppt[3];leppt[3]=dummy;   
      dummy = lepeta[2];lepeta[2]=lepeta[3];lepeta[3]=dummy;   
    }
	...
	for(int j=0;j<3&&sorted;j++){
	  if(leppt[j]<leppt[j+1]){
	    ...
	    dummy = lepeta[j];lepeta[j]=lepeta[j+1];lepeta[j+1]=dummy;    
	    sorted=false;
	  }
	}
      }
    }
    for(int j=0;j<4;j++)hReco[j]->Fill(lepeta[j]);

  }

  if(debug)printf("plotting\n");
  TString cname;cname.Form("Lepeta_%s_HMass%d",schannel.c_str(),mass);
...
void plotPtLeptons(){
  ...
  for(int im =0;im<nPoints8TeV;im++)
    for(int ich=0;ich<3;ich++)
      ...
      plotEtaLeptons(masses8TeV[im],0,ich,true,canv);
}
