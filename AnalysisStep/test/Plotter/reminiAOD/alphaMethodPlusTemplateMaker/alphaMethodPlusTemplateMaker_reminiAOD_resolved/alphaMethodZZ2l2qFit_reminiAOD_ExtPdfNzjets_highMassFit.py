...
def alphaMethod(fs,jet,cat, minMass, parser):
    ...
    #### fit only from minMass to 3.5TeV
    ...
    zjets = RooGenericPdf("zjets",mathExpression,RooArgList(l0,l1,mass))
    ...
    ttbarVZCRFit_highMass = highMassHist(ttbarVZCRFit,minMass)

    print "TT+VV Pdf dataHist"
    ttbarVZCR_templates = ROOT.RooDataHist("ttbarVZCRdataHist","ttbarVZCRdataHist",RooArgList(mass),RooFit.Import(ttbarVZCRFit_highMass,kFALSE))
    ...
    NttbarVZCR = RooRealVar("NttbarVZCR","NttbarVZCR",ttbarVZCRFit_highMass.Integral())
    ...
    dataCRup.Rebin(5)
    dataCRdn.Rebin(5)
    dataCRupFit_highMass = highMassHist(dataCRup,minMass)
    dataCRdnFit_highMass = highMassHist(dataCRdn,minMass)
    dataCRFit_highMass = highMassHist(dataCRFit,minMass)

    alphaUnc = 1-dataCRdnFit_highMass.Integral()/dataCRFit_highMass.Integral()
    
    dataCRHist = RooDataHist("dataCRdataHist","alpha-times-sbData",RooArgList(mass),RooFit.Import(dataCRFit_highMass,kFALSE))
    ...
    print 'eigen vals ',vals(0),' ',vals(1)
    ...
    ######
    l0_unc_str = "( ("+str(eigCoeff0[0])+")*"+eig0Name+"+("+str(eigCoeff0[1])+")*"+eig1Name+")"
    ######
    l1_unc_str = "( ("+str(eigCoeff1[0])+")*"+eig0Name+"+("+str(eigCoeff1[1])+")*"+eig1Name+")"
    ######
    ...
    if(dyMC=="nloDY" or jet=="merged") :
      minMass = 500
      zjetsAlphaSR = TH1D("zjetsAlphaSR","zjetsAlphaSR",300,500,3500)

    zjetsTemplates = {}
    zjetsTemplates['nominal'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,0,0,alphaUnc)
    zjetsTemplates['eig0up'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,0,1,alphaUnc)
    zjetsTemplates['eig0dn'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,0,-1,alphaUnc)
    zjetsTemplates['eig1up'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,1,1,alphaUnc)
    zjetsTemplates['eig1dn'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,1,-1,alphaUnc)
    zjetsTemplates['eig2up'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,2,1,alphaUnc)
    zjetsTemplates['eig2dn'] = templatesMaker_forTail(minMass,zjetsAlphaSR,mass,Nzjets,l0_str,l1_str,2,-1,alphaUnc)
    ...
    inputfile_fs = {}
    inputfile_fs['all'] = TFile('TMVAAndRoofitInputs_mumu_'+dyMC+'.root',"READ")
...
def templatesMaker_forTail(minMass,zjetsCR,mass,Nzjets,l0_str,l1_str,index,nSigma,alphaUnc) :
    ...
    mathExpressionDiagonal = "TMath::Exp(-"+l0_str+"*(mass/(1+"+l1_str+"*mass)))"
    zjetsDiagnol = RooGenericPdf("zjetsDiagonal",mathExpressionDiagonal,RooArgList(eig0,eig1,mass))
    ...
    for i in range(1,zjetsCRtmp.GetXaxis().GetNbins()+1):
        ...
        if(minMass<high):
          ...
          if(index==2) :
            k = 1+nSigma*pow(pow(Nzjets.getError()/Nzjets.getVal(),2)+pow(alphaUnc,2),0.5)
            if(nSigma<0):
              k = 1/(1-nSigma*pow(pow(Nzjets.getError()/Nzjets.getVal(),2)+pow(alphaUnc,2),0.5))
            zjetsCRtmp.SetBinContent(i,rfvSigRateZjetsDiagnol_i.getVal()*k)
...
def highMassHist(histCRFit,minMass) :
    bins_highMass=[]
    for i in range(1,histCRFit.GetXaxis().GetNbins()+1):
        low = histCRFit.GetXaxis().GetBinLowEdge(i)
        high = histCRFit.GetXaxis().GetBinUpEdge(i)
        if(low>=minMass) :
          bins_highMass.append(low)

    bins_highMass.append(histCRFit.GetXaxis().GetBinUpEdge(histCRFit.GetXaxis().GetNbins()))
    nbins = len(bins_highMass)-1

    print 'bins_highMass'
    print bins_highMass

    histCRFit_highMass = TH1F(histCRFit.GetName()+"_highMass",histCRFit.GetTitle(),nbins,array('d',bins_highMass))    
    nmiss = histCRFit.GetXaxis().GetNbins()-histCRFit_highMass.GetXaxis().GetNbins()
    print 'nmiss ',nmiss
    for j in range(1,histCRFit_highMass.GetXaxis().GetNbins()+1):
        print 'bincontent ',histCRFit.GetBinContent(i+nmiss),' binerror ',histCRFit.GetBinError(j+nmiss)
        histCRFit_highMass.SetBinContent(j, histCRFit.GetBinContent(j+nmiss))
        histCRFit_highMass.SetBinError(j, histCRFit.GetBinError(j+nmiss))

    c = TCanvas("c","c",10,10,1000,800)
    c.cd()
    histCRFit_highMass.Draw("ep3")
    c.SaveAs(histCRFit_highMass.GetName()+".png")

    return histCRFit_highMass
...
def Run():
    ...
    alphaMethod('ee','resolved','', 800,parser)
    alphaMethod('mumu','resolved','', 800,parser)
