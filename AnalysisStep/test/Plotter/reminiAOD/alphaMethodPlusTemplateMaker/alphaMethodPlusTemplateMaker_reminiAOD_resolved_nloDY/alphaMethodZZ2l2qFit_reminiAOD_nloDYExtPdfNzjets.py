...
def parseOptions():
    ...
    # sample opt
    parser.add_option("--dyMC", dest="dyMC",default='nloDY',help="Type of DY sample")
...
def ZJetsNorm(low,high, p0,l0,p1,l1):
    # norm of Zjets
    bkgRate_zjets_TString_Exponential1 = "(1/"+l0+")*(TMath::Exp("+p0+"-"+l0+"*"+str(low)+")-TMath::Exp("+p0+"-"+l0+"*"+str(high)+"))"
...
def alphaMethod(fs,jet,cat, minMass, parser):
    ...
    if(cat=='btag') :
      nRebin=2
      bins_alpha=[500,550,600,650,700,800,1000,1100,1300,1500,1800,3500]
      if(dyMC=='nloDY') :
        bins_alpha=[500,550,600,700,800,1000,1500,3500]
    ...
    if(cat=='vbf') :
      ...
      if(dyMC=='nloDY') :
        bins_alpha=[500,550,600,700,800,1000,1200,3500]
    ...
    print "alphaUnc ",alphaUnc
    print "Nzjets.getError()/Nzjets.getVal() ",Nzjets.getError()/Nzjets.getVal()
    print "alphaUnc*alphaUnc-pow(Nzjets.getError()/Nzjets.getVal(),2) ",alphaUnc*alphaUnc-pow(Nzjets.getError()/Nzjets.getVal(),2)
    zjetsTemplates['eig3dn'].Scale(pow(1-alphaUnc*alphaUnc-pow(Nzjets.getError()/Nzjets.getVal(),2),0.5))
    if(dyMC=="nloDY") :
       zjetsTemplates['eig3dn'].Scale(1/(pow(1+alphaUnc*alphaUnc+pow(Nzjets.getError()/Nzjets.getVal(),2),0.5)))
