#!/usr/bin/env python3
# Script Imported from PhysicsTools/NanoAOD/test/inspectNanoFile.py, with some modifications for H4l:
# -Add AllEvents tree
# -reorder trees so that Events is at the beginning
# -fix broken "back to top" links in Markdown
# -Add initial title
...
def writeSizeReport(fileData, trees, stream):
    ...
    for treename, (survey, _) in surveys.items():
        ...
        for s in survey:
            ...
            stream.write("<td class=\"img\"><img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/blue-dot.gif' width='%d' height='%d' /></td>" % (s['tot']/treetotal*200,10))
        ...
        stream.write("<td class=\"img\"><img src=\"https://gpetrucc.web.cern.ch/gpetrucc/micro/green-dot.gif\" width='%d' height='10' />" % ( treetotal/filesize*100.0))
        ...
        if treename == "Events":
            ...
            stream.write("<td class=\"img\"><img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' /></td>" % ( (filesize-treetotal)/filesize * 100, 10 ))
    ...
    if len(surveys) > 1:
        ...
        stream.write("<td class=\"img\"><img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' /></td>" % ( (filesize-runningtotal)/filesize * 100, 10 ))
...
def writeMarkdownSizeReport(fileData, trees, stream):
    ...
    for treename, survey in surveys.items():
        ...
        for s in survey:
            ...
            stream.write("| <img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/blue-dot.gif' width='%d' height='%d' />" % (s['tot'] / treetotal * 200, 10))
        ...
        stream.write("| | <img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/green-dot.gif' width='%d' height='%d' />" % (treetotal / filesize * 100.0, 10))
        ...
        if treename == "Events":
            ...
            stream.write("| | <img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' />" % ((filesize - treetotal) / filesize * 100, 10))
    ...
    if len(surveys) > 1:
        ...
        stream.write("| | <img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/red-dot.gif' width='%d' height='%d' />" % ((filesize - runningtotal) / filesize * 100, 10))
    ...
    for treename, survey in surveys.items():
        ...
        for s in sorted(survey, key=lambda s: s['name']):
            ...
            for b in sorted(subs, key = lambda s: - s['tot']):
                ...
                stream.write("| <img src='https://gpetrucc.web.cern.ch/gpetrucc/micro/blue-dot.gif' width='%d' height='%d' />" % (b['tot'] / s['tot'] * 200, 10))
                stream.write("| %.1f%% |\n" % (b['tot'] / s['tot'] * 100.0))
            stream.write("\n")

def writeMarkdownDocReport(trees, stream):
    stream.write("# H4l nanoAOD file format documentation\n")
    stream.write("This document is generated automatically with:  \n")
    stream.write("`./inspectNanoFile.py ZZ4lAnalysis.root --docmd ZZ4lNanoDoc.md`\n\n")
    stream.write("Jump to:\n\n")    
    for treename, treeData in trees.items():
        stream.write("* [%s tree](#%s-tree-content)\n" %(treename,treename.lower()))  

    for treename, treeData in trees.items():
        stream.write("## %s tree content\n" % treename)
